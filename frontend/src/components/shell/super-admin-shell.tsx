...
import { SUPER_NAV, type NavItem } from "@/lib/navigation";
    ...
    crumbs.push({
      ...
      href:
        index === segments.length - 1
          ? undefined
          : `/super-admin/${segments.slice(0, index + 1).join("/")}`,
    });
  });
  return crumbs;
}
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 lg:grid lg:grid-cols-[16.5rem_1fr]">
      <aside className="sticky top-0 hidden h-screen flex-col bg-slate-900 lg:flex">
        ...
        <div className="flex-1 overflow-y-auto py-3">
          ...
          <SidebarNav items={SUPER_NAV} pathname={pathname} />
        </div>

        <div className="border-t border-white/5 p-4">
          <DropdownMenu>
            <DropdownMenuTrigger className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left transition-colors hover:bg-white/5 outline-none">
            ...
            <DropdownMenuContent
              ...
              side="top"
              sideOffset={8}
              className="z-[9999] w-56 border-slate-700 bg-slate-800 text-slate-200 shadow-2xl shadow-black/50"
            >
      ...
      <div className="flex min-h-screen flex-col">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-slate-200/60 bg-white/70 px-4 backdrop-blur-xl dark:border-slate-800/60 dark:bg-slate-950/70 md:px-8">
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            <SheetContent
              side="left"
              className="w-80 border-white/5 bg-slate-900 p-0"
            >
            ...
            {breadcrumbs.map((crumb, index) => (
              <React.Fragment key={`${crumb.label}-${index}`}>
                ...
                {crumb.href ? (
                  <Link
                    href={crumb.href}
                    className="text-slate-500 transition-colors hover:text-slate-800 dark:hover:text-slate-200"
                  >
                    {crumb.label}
                  </Link>
                ) : (
          ...
          <h1 className="truncate font-heading text-base font-bold tracking-tight md:hidden">
