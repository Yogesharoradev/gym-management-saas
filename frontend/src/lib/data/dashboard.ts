...
export async function getGymDashboardData(gymId: string): Promise<DashboardData> {
  ...
  const [totalMembers, activeMembers, inactiveMembers, expiringSoon, expiredMembers, todayAttendance, todayCollectionResult, pendingResult, expiringDocs, revenueDocs] = await Promise.all([
    ...
    MembershipModel.countDocuments({ gymId: tenantId, status: MEMBERSHIP_STATUS.ACTIVE, startDate: { $lte: now }, endDate: { $gte: todayStart, $lte: sevenDaysFromNow } }),
    ...
    MembershipModel.aggregate<{ _id: unknown; endDate: Date; member: Array<{ name: string }>; plan: Array<{ name: string }> }>([{ $match: { gymId: tenantId, status: MEMBERSHIP_STATUS.ACTIVE, startDate: { $lte: now }, endDate: { $gte: todayStart, $lte: sevenDaysFromNow } } }, { $sort: { endDate: 1 } }, { $limit: 6 }, { $lookup: { from: "members", localField: "memberId", foreignField: "_id", as: "member" } }, { $lookup: { from: "membershipplans", localField: "planId", foreignField: "_id", as: "plan" } }]),
  ...
  const expiringMembers = expiringDocs.map((item) => { const days = Math.max(0, Math.ceil((new Date(item.endDate).getTime() - todayStart.getTime()) / 86400000)); return { id: String(item._id), name: item.member[0]?.name ?? "Unknown member", plan: item.plan[0]?.name ?? "Membership", endsIn: days === 0 ? "Today" : `${days} ${days === 1 ? "day" : "days"}` }; });
