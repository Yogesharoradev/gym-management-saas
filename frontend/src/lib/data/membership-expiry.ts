...
function serialize(membership: IMembership, member: { _id: Types.ObjectId; name: string; phone: string; status: string }, plan: IMembershipPlan, today: Date): ExpiryMembership {
  ...
  return { id: membership._id.toString(), member: { id: member._id.toString(), name: member.name, phone: member.phone, status: member.status }, plan: { id: plan._id.toString(), name: plan.name, duration: plan.duration, durationUnit: plan.durationUnit, price: plan.price, isActive: plan.isActive }, startDate: membership.startDate.toISOString(), endDate: membership.endDate.toISOString(), amount: membership.amount, weightAtStart: membership.weightAtStart, status: membership.status, daysLeft, bucket };
}
...
export async function listMembershipExpiry(gymId: string, bucket?: ExpiryBucket, query?: string): Promise<{ memberships: ExpiryMembership[]; summary: ExpirySummary }> {
  ...
  const dateFilter: Record<string, unknown> = { $or: [{ endDate: { $gte: today, $lte: inThirtyDays } }, { endDate: { $lt: today } }] };
  ...
  const summary: ExpirySummary = { expired: all.filter((item) => item.bucket === "EXPIRED").length, today: all.filter((item) => item.bucket === "TODAY").length, threeDays: all.filter((item) => item.bucket === "THREE_DAYS").length, sevenDays: all.filter((item) => item.bucket === "SEVEN_DAYS").length, thirtyDays: all.filter((item) => item.daysLeft >= 8 && item.daysLeft <= 30).length };
