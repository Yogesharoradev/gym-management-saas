...
export function MemberForm({ mode, member }: MemberFormProps) {
  ...
  const [form, setForm] = React.useState<FormState>({
    ...
    joiningDate: toInputDate(member?.joiningDate) || new Date().toISOString().slice(0, 10),
  });
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try {
      const response = await fetch(mode === "create" ? "/api/members" : `/api/members/${member?.id}`, {
    ...
    <form onSubmit={submit} className="space-y-6">
      ...
      <section className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm sm:p-6">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div><h2 className="font-heading text-base font-bold">Personal details</h2><p className="text-xs text-muted-foreground">Keep the member profile accurate and easy to identify.</p></div>
        </div>
      ...
      <section className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm sm:p-6">
        <div className="mb-6"><h2 className="font-heading text-base font-bold">Contact & joining</h2><p className="text-xs text-muted-foreground">Useful information for communication and emergencies.</p></div>
        <div className="grid gap-5 sm:grid-cols-2">
          <div className="space-y-2 sm:col-span-2"><Label htmlFor="member-address">Address</Label><Input id="member-address" value={form.address} onChange={(e) => update("address", e.target.value)} placeholder="Street, area, city" /></div>
          <div className="space-y-2"><Label htmlFor="member-emergency">Emergency contact</Label><Input id="member-emergency" value={form.emergencyContact} onChange={(e) => update("emergencyContact", e.target.value)} placeholder="Name / phone" /></div>
          <div className="space-y-2"><Label htmlFor="member-joining">Joining date *</Label><Input id="member-joining" type="date" value={form.joiningDate} onChange={(e) => update("joiningDate", e.target.value)} required /></div>
        </div>
      </section>

      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
        <Button type="button" variant="outline" onClick={() => router.back()} className="w-full sm:w-auto"><ArrowLeft className="mr-2 h-4 w-4" />Cancel</Button>
        <Button type="submit" disabled={saving} className="w-full sm:w-auto"><>{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}</>{saving ? "Saving…" : mode === "create" ? "Add member" : "Save changes"}</Button>
      </div>
    </form>
  );
}
