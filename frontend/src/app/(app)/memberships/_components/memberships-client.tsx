...
import { CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Clock3, CreditCard, Edit3, IndianRupee, Plus, Search, Sparkles, UserRound, X } from "lucide-react";
...
export function MembershipsClient() {
  ...
  async function cancelMembership(id: string): Promise<void> {
    const response = await fetch(`/api/memberships/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: MEMBERSHIP_STATUS.CANCELLED }) });
    if (response.ok) await loadMemberships();
  }
  ...
  return <div className="space-y-6 sm:space-y-7">
    ...
    {planOpen ? <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-950/35 p-0 backdrop-blur-sm sm:items-center sm:p-4"><Card className="max-h-[92vh] w-full overflow-y-auto rounded-t-[1.75rem] border-0 p-5 shadow-2xl sm:max-w-lg sm:rounded-[1.5rem] sm:border sm:p-6"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-emerald-600">Membership plan</p><h2 className="mt-1 text-xl font-bold text-slate-900">{editingPlan ? "Edit plan" : "Create a plan"}</h2></div><Button variant="ghost" size="icon-sm" onClick={() => setPlanOpen(false)}><X className="h-4 w-4" /></Button></div><form onSubmit={(event) => void savePlan(event)} className="mt-6 space-y-4"><label className="block"><span className="text-sm font-semibold text-slate-700">Plan name</span><Input required value={planForm.name} onChange={(e) => setPlanForm({ ...planForm, name: e.target.value })} placeholder="Monthly Fitness" className="mt-1.5 h-11 rounded-xl" /></label><div className="grid grid-cols-2 gap-3"><label><span className="text-sm font-semibold text-slate-700">Duration</span><Input required type="number" min="1" value={planForm.duration} onChange={(e) => setPlanForm({ ...planForm, duration: e.target.value })} className="mt-1.5 h-11 rounded-xl" /></label><label><span className="text-sm font-semibold text-slate-700">Unit</span><select value={planForm.durationUnit} onChange={(e) => setPlanForm({ ...planForm, durationUnit: e.target.value as DurationUnit })} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-400"><option value={DURATION_UNIT.DAY}>Days</option><option value={DURATION_UNIT.MONTH}>Months</option><option value={DURATION_UNIT.YEAR}>Years</option></select></label></div><label><span className="text-sm font-semibold text-slate-700">Price</span><div className="relative mt-1.5"><IndianRupee className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><Input required type="number" min="0" value={planForm.price} onChange={(e) => setPlanForm({ ...planForm, price: e.target.value })} placeholder="1500" className="h-11 rounded-xl pl-9" /></div></label><label><span className="text-sm font-semibold text-slate-700">Description <span className="font-normal text-slate-400">(optional)</span></span><textarea value={planForm.description} onChange={(e) => setPlanForm({ ...planForm, description: e.target.value })} placeholder="Unlimited gym access..." className="mt-1.5 min-h-24 w-full rounded-xl border border-slate-200 bg-white p-3 text-sm outline-none focus:border-emerald-400" /></label><div className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end"><Button type="button" variant="outline" onClick={() => setPlanOpen(false)} className="rounded-xl">Cancel</Button><Button disabled={saving} type="submit" className="rounded-xl bg-emerald-600 hover:bg-emerald-700">{saving ? "Saving..." : editingPlan ? "Save changes" : "Create plan"}</Button></div></form></Card></div> : null}

    {assignOpen ? <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-950/35 p-0 backdrop-blur-sm sm:items-center sm:p-4"><Card className="max-h-[94vh] w-full overflow-y-auto rounded-t-[1.75rem] border-0 p-5 shadow-2xl sm:max-w-xl sm:rounded-[1.5rem] sm:border sm:p-6"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-emerald-600">New membership</p><h2 className="mt-1 text-xl font-bold text-slate-900">Assign a plan</h2></div><Button variant="ghost" size="icon-sm" onClick={() => setAssignOpen(false)}><X className="h-4 w-4" /></Button></div><form onSubmit={(event) => void assignMembership(event)} className="mt-6 space-y-4"><label><span className="text-sm font-semibold text-slate-700">Member</span><div className="relative mt-1.5"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><Input value={memberQuery} onChange={(e) => setMemberQuery(e.target.value)} placeholder="Search member..." className="h-11 rounded-xl pl-9" /></div><select required value={assignForm.memberId} onChange={(e) => setAssignForm({ ...assignForm, memberId: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-400"><option value="">Select member</option>{members.map((member) => <option key={member.id} value={member.id}>{member.name} · {member.phone}</option>)}</select></label><label><span className="text-sm font-semibold text-slate-700">Membership plan</span><select required value={assignForm.planId} onChange={(e) => selectPlan(e.target.value)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-400"><option value="">Select plan</option>{activePlans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name} · {money.format(plan.price)}</option>)}</select></label><div className="grid gap-3 sm:grid-cols-2"><label><span className="text-sm font-semibold text-slate-700">Start date</span><Input required type="date" value={assignForm.startDate} onChange={(e) => selectStartDate(e.target.value)} className="mt-1.5 h-11 rounded-xl" /></label><label><span className="text-sm font-semibold text-slate-700">End date</span><Input required type="date" value={assignForm.endDate} onChange={(e) => setAssignForm({ ...assignForm, endDate: e.target.value })} className="mt-1.5 h-11 rounded-xl" /></label></div><div className="grid gap-3 sm:grid-cols-2"><label><span className="text-sm font-semibold text-slate-700">Amount</span><div className="relative mt-1.5"><IndianRupee className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><Input required type="number" min="0" value={assignForm.amount} onChange={(e) => setAssignForm({ ...assignForm, amount: e.target.value })} className="h-11 rounded-xl pl-9" /></div></label><label><span className="text-sm font-semibold text-slate-700">Starting weight <span className="font-normal text-slate-400">(kg)</span></span><Input type="number" min="0" step="0.1" value={assignForm.weightAtStart} onChange={(e) => setAssignForm({ ...assignForm, weightAtStart: e.target.value })} placeholder="e.g. 72.5" className="mt-1.5 h-11 rounded-xl" /></label></div><div className="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-4"><div className="flex gap-3"><CalendarDays className="mt-0.5 h-5 w-5 text-emerald-600" /><div><p className="text-sm font-semibold text-slate-800">Membership starts on {assignForm.startDate ? formatDate(assignForm.startDate) : "—"}</p><p className="mt-1 text-xs leading-5 text-slate-500">The selected plan automatically suggests the end date. You can adjust it before saving.</p></div></div></div><div className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end"><Button type="button" variant="outline" onClick={() => setAssignOpen(false)} className="rounded-xl">Cancel</Button><Button disabled={saving || !activePlans.length} type="submit" className="rounded-xl bg-emerald-600 hover:bg-emerald-700">{saving ? "Assigning..." : "Assign membership"}</Button></div></form></Card></div> : null}
  </div>;
}
