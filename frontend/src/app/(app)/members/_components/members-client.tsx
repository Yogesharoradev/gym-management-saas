  ...
  return (
    <div className="space-y-6 sm:space-y-7">
      <section className="relative overflow-hidden rounded-[1.75rem] border border-emerald-100 bg-gradient-to-br from-white via-emerald-50/70 to-cyan-50/70 p-5 shadow-[0_18px_50px_rgba(16,185,129,0.08)] sm:p-7 lg:p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="font-heading text-3xl font-black tracking-[-0.035em] text-slate-900 sm:text-4xl">
              Your members, beautifully organised.
            </h1>
            <p className="mt-2.5 max-w-xl text-sm leading-6 text-slate-500">
              Find members quickly, see their membership at a glance, and keep
              your gym directory up to date.
            </p>
          </div>
          <div className="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
            <Button className="inline-flex h-11 w-full flex-row items-center justify-center gap-2 whitespace-nowrap rounded-xl bg-emerald-600 px-5 font-semibold text-white shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 sm:w-auto">
              <Link
                href="/members/new"
        ...
        <div className="relative mt-6 grid grid-cols-2 gap-2 sm:grid-cols-5 sm:gap-3">
          {[
            ...
            { label: "With membership", value: stats.withMembership },
          ].map((item) => (
            <div
              key={item.label}
              className="rounded-2xl border border-white/80 bg-white/75 p-3 backdrop-blur-sm sm:p-4"
            >
      ...
      <Card className="rounded-[1.5rem] border-slate-200/80 bg-white shadow-[0_12px_40px_rgba(15,23,42,0.05)]">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="h-11 rounded-xl border-slate-200 bg-slate-50/70 pl-10 pr-10 text-sm focus:border-emerald-300 focus:bg-white focus:ring-4 focus:ring-emerald-500/5"
              />
              {query ? (
                <button
                  type="button"
        ...
        {error ? (
          <div className="m-4 flex items-center gap-2 rounded-xl border border-rose-100 bg-rose-50 px-4 py-3 text-sm text-rose-700">
            <X className="h-4 w-4" />
            {error}
          </div>
        ) : null}
        {loading ? (
          ...
          <div className="flex min-h-80 flex-col items-center justify-center px-5 py-12 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-50 to-cyan-50 text-emerald-600">
              <UserRound className="h-7 w-7" />
            </div>
            <h2 className="mt-4 font-heading text-xl font-bold text-slate-800">
              {query || status !== "ALL"
                ? "No members found"
                : "Start building your member base"}
            </h2>
            <p className="mt-1 max-w-sm text-sm leading-6 text-slate-500">
              {query || status !== "ALL"
                ? "Try a different search or status filter."
                : "Add your first member and keep everything about their gym journey in one place."}
            </p>
            {!query && status === "ALL" ? (
              <Button className="mt-5 inline-flex flex-row items-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-700">
                <Link
                  href="/members/new"
              ...
              <table className="w-full min-w-[850px] text-sm">
                ...
                <tbody>
                  {members.map((member) => (
                    <tr
                      key={member.id}
                      className="group border-b border-slate-100 last:border-0 transition-colors hover:bg-emerald-50/30"
                    >
                      <td className="px-5 py-4">
                        ...
                        >
                          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-100 to-cyan-100 text-xs font-black text-emerald-700">
                            {initials(member.name)}
                          </span>
                          <span className="min-w-0">
                            <span className="block truncate font-semibold text-slate-800 group-hover:text-emerald-700">
                              {member.name}
                            </span>
                            <span className="mt-0.5 block text-xs text-slate-400">
                              Joined {formatDate(member.joiningDate, true)}
                            </span>
                          </span>
                        </Link>
                      </td>
                        ...
                        {member.membership ? (
                          <>
                            ...
                            <p className="mt-0.5 text-xs text-slate-400">
                              Ends {formatDate(member.membership.endDate)}
                            </p>
                          </>
                        ) : (
                      ...
                      <td className="px-4 py-4">
                        <Badge
                          variant="outline"
                          className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${statusClass(member.status)}`}
                        >
                          {member.status}
                        </Badge>
                      </td>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <Badge
                          variant="outline"
                          className={`rounded-full text-[10px] ${statusClass(member.status)}`}
                        >
                          {member.status}
                        </Badge>
                      </div>
                      <div className="mt-3 rounded-2xl border border-slate-100 bg-gradient-to-br from-slate-50 to-emerald-50/50 p-3.5">
                        ...
                        {member.membership ? (
                          <p className="mt-1 text-xs text-slate-400">
                            Valid until{" "}
                            {formatDate(member.membership.endDate, true)}
                          </p>
                        ) : null}
                      </div>
                      <div className="mt-3 flex gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          className="inline-flex h-9 flex-1 flex-row items-center justify-center gap-1.5 rounded-xl border-slate-200 whitespace-nowrap"
                        >
                          <Link
                            href={`/members/${member.id}`}
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          className="inline-flex h-9 w-10 shrink-0 flex-row items-center justify-center rounded-xl border-slate-200 p-0"
                          aria-label={`Actions for ${member.name}`}
                          onClick={(event) => openMenu(event, member.id)}
                        >
                          <MoreHorizontal className="h-4 w-4" />
            ...
            <div className="flex flex-col gap-3 border-t border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  className="inline-flex flex-row items-center gap-1.5 rounded-xl whitespace-nowrap"
                >
                  <ChevronLeft className="h-4 w-4" />
                  Previous
                </Button>
                <span className="flex h-9 min-w-9 items-center justify-center rounded-xl bg-slate-100 px-3 text-xs font-bold text-slate-600">
                  {page} / {pages}
                </span>
                <Button
                  variant="outline"
      ...
      {selectedMember && menuPosition && typeof document !== "undefined"
        ? createPortal(
            <div
              className="fixed z-[100] w-[190px] rounded-2xl border border-slate-200 bg-white p-1.5 text-left shadow-2xl shadow-slate-900/15"
            ...
            >
              <Link
                ...
                className="flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-emerald-50 hover:text-emerald-700"
              >
              ...
              <Link
                ...
                className="flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-blue-50 hover:text-blue-700"
              >
              ...
              <button
                ...
                onClick={() => void toggleStatus(selectedMember)}
                className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-sm font-medium text-rose-600 hover:bg-rose-50 disabled:opacity-50"
              >
