...
import { CheckCircle2, ChevronLeft, ChevronRight, Eye, MoreHorizontal, Pencil, Plus, Search, Sparkles, UserRound, UserRoundX, X } from "lucide-react";
...
export function MembersClient({ actions }: { actions?: React.ReactNode }) {
  ...
  return <div className="space-y-6 sm:space-y-7">
    <section className="relative overflow-hidden rounded-[1.75rem] border border-emerald-100 bg-gradient-to-br from-white via-emerald-50/70 to-cyan-50/70 p-5 shadow-[0_18px_50px_rgba(16,185,129,0.08)] sm:p-7 lg:p-8">
      ...
      <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-2xl"><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/75 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-emerald-700"><Sparkles className="h-3.5 w-3.5" />Member management</div><h1 className="font-heading text-3xl font-black tracking-[-0.035em] text-slate-900 sm:text-4xl">Your members, beautifully organised.</h1><p className="mt-2.5 max-w-xl text-sm leading-6 text-slate-500">Find members quickly, see their membership at a glance, and keep your gym directory up to date.</p></div><div className="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:items-center"><Button asChild className="inline-flex h-11 w-full flex-row items-center justify-center gap-2 whitespace-nowrap rounded-xl bg-emerald-600 px-5 font-semibold text-white shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 sm:w-auto"><Link href="/members/new" className="inline-flex items-center justify-center gap-2 whitespace-nowrap"><Plus className="h-4 w-4 shrink-0" />Add member</Link></Button>{actions}</div></div>
    ...
    {selectedMember && menuPosition && typeof document !== "undefined" ? createPortal(<div className="fixed z-[100] w-[190px] rounded-2xl border border-slate-200 bg-white p-1.5 text-left shadow-2xl shadow-slate-900/15" style={{ top: menuPosition.top, left: menuPosition.left }} role="menu"><Link href={`/members/${selectedMember.id}`} onClick={() => { setMenuId(null); setMenuPosition(null); }} className="flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-emerald-50 hover:text-emerald-700"><Eye className="h-4 w-4 text-emerald-600" />View profile</Link><Link href={`/members/${selectedMember.id}/edit`} onClick={() => { setMenuId(null); setMenuPosition(null); }} className="flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-blue-50 hover:text-blue-700"><Pencil className="h-4 w-4 text-blue-500" />Edit member</Link><button type="button" disabled={updatingId === selectedMember.id} onClick={() => void toggleStatus(selectedMember)} className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-sm font-medium text-rose-600 hover:bg-rose-50 disabled:opacity-50"><UserRoundX className="h-4 w-4" />{selectedMember.status === "ACTIVE" ? "Deactivate" : "Activate"}</button></div>, document.body) : null}
  </div>;
}
