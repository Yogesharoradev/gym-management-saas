      ...
      <section className="relative overflow-hidden rounded-[1.75rem] border border-emerald-100 bg-gradient-to-br from-white via-emerald-50/70 to-cyan-50/70 p-5 shadow-[0_18px_50px_rgba(16,185,129,0.08)] sm:p-7 lg:p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center">
          <div className="flex items-center gap-4 sm:gap-5">
            <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-[1.5rem] bg-gradient-to-br from-emerald-500 to-teal-500 text-2xl font-black text-white shadow-xl shadow-emerald-500/20 sm:h-24 sm:w-24 sm:text-3xl">
              {initials}
            </div>
            <div className="min-w-0">
              ...
              <p className="mt-1.5 text-sm text-slate-500">
                Member since {formatDate(member.joiningDate)} · {member.phone}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:ml-auto">
            <div className="rounded-2xl border border-white/80 bg-white px-4 py-3 shadow-sm">
              ...
              <p className="mt-1 max-w-32 truncate text-sm font-bold text-slate-800">
                {currentMembership?.plan ?? "Not assigned"}
              </p>
            </div>
            <div className="rounded-2xl border border-white/80 bg-white px-4 py-3 shadow-sm">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
            ...
            <div className="col-span-2 rounded-2xl border border-white/80 bg-white px-4 py-3 shadow-sm sm:col-span-1">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
                Starting weight
              </p>
              <p className="mt-1 text-sm font-bold text-slate-800">
                {currentMembership?.weightAtStart != null
                  ? `${currentMembership.weightAtStart} kg`
                  : "Not added"}
              </p>
            </div>
          </div>
        ...
        <section className="flex flex-col gap-3 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
          <div>
            <p className="text-sm font-bold text-rose-800">
              ...
              {formatDate(currentMembership?.endDate ?? null)}
            </p>
            <p className="mt-0.5 text-xs text-rose-600">
              Renew the membership to keep the member active in your gym.
            </p>
          </div>
          <Button className="h-10 shrink-0 rounded-xl bg-rose-600 hover:bg-rose-700">
            <Link href="/expiry">Open expiry</Link>
          </Button>
        </section>
      ) : null}
          ...
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Everything you need to know about this member.
              </p>
            </div>
          </div>
        ...
        <section className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_10px_35px_rgba(15,23,42,0.04)] sm:p-7">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Latest plan and membership snapshot.
              </p>
            </div>
          </div>
          {currentMembership ? (
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-5">
              <div className="flex items-center justify-between gap-3">
                <Badge
                  variant="outline"
                  className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${statusClass(currentMembership.status)}`}
                >
              ...
              <div className="mt-5 space-y-3 border-t border-emerald-100 pt-4 text-sm">
            ...
            <div className="flex min-h-48 flex-col items-center justify-center rounded-2xl border border-dashed border-cyan-200 bg-cyan-50/30 px-4 text-center">
              ...
              <p className="mt-1 max-w-xs text-xs leading-5 text-slate-400">
                Assign a plan from Memberships when you are ready.
              </p>
              <Button
                size="sm"
                className="mt-4 rounded-xl bg-emerald-600 hover:bg-emerald-700"
              >
                <Link href="/memberships">Assign membership</Link>
              </Button>
            </div>
          )}
        </section>
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Every membership period stays here for a complete record.
              </p>
            </div>
          </div>
          <Badge
            variant="outline"
            className="w-fit rounded-full border-slate-200 bg-slate-50 text-slate-500"
          >
            {membershipHistory.length}{" "}
            {membershipHistory.length === 1 ? "membership" : "memberships"}
          </Badge>
        </div>
        {membershipHistory.length ? (
            ...
            {membershipHistory.map((membership, index) => (
              ...
              >
                <div className="flex min-w-0 gap-3">
                  <div
                    className={`mt-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${index === 0 ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-500"}`}
                  >
                    {index === 0 ? (
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Badge
                        variant="outline"
                        className={`rounded-full px-2 py-0.5 text-[9px] font-bold ${statusClass(membership.status)}`}
                      >
                        {membership.status}
                      </Badge>
                      {index === 0 ? (
