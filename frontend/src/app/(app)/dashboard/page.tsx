  ...
  const stats = [
    {
      key: "total-members",
      label: "Total Members",
      value: dashboard.totalMembers.toLocaleString("en-IN"),
    ...
    },
    {
      ...
      value: dashboard.activeMembers.toLocaleString("en-IN"),
      helper: "Member status is active",
      icon: UserCheck,
      accent: true,
    },
    {
      ...
      value: dashboard.inactiveMembers.toLocaleString("en-IN"),
      helper: "Member status is inactive",
      icon: UserX,
      tone: "rose" as const,
    },
    {
      key: "expiring-soon",
      label: "Expiring Soon",
      value: dashboard.expiringSoon.toLocaleString("en-IN"),
      helper: "Memberships in next 7 days",
      icon: Clock3,
      tone: "amber" as const,
    },
    {
      key: "expired-members",
      label: "Expired",
      value: dashboard.expiredMembers.toLocaleString("en-IN"),
      helper: "Memberships already ended",
      icon: UserX,
    ...
    <div className="space-y-6 sm:space-y-7">
      <section className="relative overflow-hidden rounded-[24px] border border-emerald-100 bg-gradient-to-br from-white via-white to-emerald-50/70 p-6 shadow-[0_12px_40px_rgba(15,23,42,0.05)] sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            ...
            <h1 className="mt-2 font-heading text-3xl font-black tracking-[-0.04em] text-slate-900 sm:text-4xl">
              {firstName} <span aria-hidden="true">👋</span>
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500 sm:text-[15px]">
              Your daily gym overview, with the members who need attention
              first.
            </p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-amber-950">
                Your Fitaah subscription has expired
              </p>
              <p className="mt-1 text-sm leading-6 text-amber-800/80">
                ...
                {gym.subscriptionEndDate
                  ...
                  : "the expiry date"}
                . You are in the {access.graceDaysRemaining}-day grace period.
                Please contact the administrator before access is paused.
              </p>
            </div>
          </div>
        </div>
      ...
      <section>
        <div className="mb-3 flex items-end justify-between gap-3 px-0.5">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-400">
              Membership actions based on today&apos;s date
            </p>
          </div>
          <Link
            href="/expiry"
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Link
            href="/expiry"
            className="group rounded-2xl border border-rose-100 bg-gradient-to-br from-white to-rose-50/60 p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          >
            ...
            <p className="mt-4 font-bold text-slate-800">Expired memberships</p>
            <p className="mt-1 text-xs leading-5 text-slate-500">
              Members whose membership end date has already passed.
            </p>
            <span className="mt-3 inline-flex items-center text-xs font-bold text-rose-600">
              Review and renew{" "}
          ...
          <Link
            href="/expiry"
            className="group rounded-2xl border border-amber-100 bg-gradient-to-br from-white to-amber-50/60 p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          >
            <div className="flex items-center justify-between">
            ...
            <p className="mt-4 font-bold text-slate-800">Expiring in 7 days</p>
            <p className="mt-1 text-xs leading-5 text-slate-500">
              Members approaching their membership end date.
            </p>
            <span className="mt-3 inline-flex items-center text-xs font-bold text-amber-700">
              Plan renewals{" "}
              <ArrowRight className="ml-1 h-3.5 w-3.5 transition group-hover:translate-x-0.5" />
            </span>
          </Link>
        ...
        <p className="mt-2 text-[11px] text-slate-400">
          {attentionTotal === 0
            ? "No membership actions are due right now."
            : `${attentionTotal} membership${attentionTotal === 1 ? "" : "s"} need attention.`}
        </p>
      </section>
      ...
      <section>
        <div className="mb-3 px-0.5">
          ...
          <p className="mt-0.5 text-xs text-slate-400">Live member metrics</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1.5fr)_minmax(320px,0.9fr)]">
        <Card className="rounded-[22px] border-slate-200/80 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.04)]">
          <CardHeader className="flex flex-row items-start justify-between space-y-0 px-5 sm:px-6">
            <div>
              <CardTitle className="text-base font-bold text-slate-900">
                Expiring memberships
              </CardTitle>
              <p className="mt-1 text-xs text-slate-400">
                The next 7 days, ordered by urgency
              </p>
            </div>
          ...
          <CardContent className="px-5 pb-5 sm:px-6 sm:pb-6">
            {dashboard.expiringMembers.length > 0 ? (
              <div className="space-y-2">
                {dashboard.expiringMembers.map((member) => (
                  <div
                    key={member.id}
                  ...
                  >
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      <Link
                        href="/expiry"
                        className="rounded-lg p-1.5 text-slate-400 hover:bg-white hover:text-emerald-600"
                        aria-label={`Open renewal workspace for ${member.name}`}
                      >
                ...
                <Link href="/expiry" className={`${actionLinkClass} mt-2 h-10`}>
                  View all expiry actions
                  <ArrowRight className="h-4 w-4 text-slate-400" />
                </Link>
              </div>
            ) : (
              <div className="flex min-h-44 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/50 px-4 text-center">
        ...
        <Card className="rounded-[22px] border-slate-200/80 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.04)]">
          <CardHeader>
            ...
            <p className="mt-1 text-xs text-slate-400">
              Common tasks for the gym desk
            </p>
          </CardHeader>
          <CardContent className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1">
            <Link href="/members/new" className={`${actionLinkClass} h-11`}>
            ...
            <Link href="/memberships" className={`${actionLinkClass} h-11`}>
              <span className="flex items-center gap-2">
            ...
            <Link href="/expiry" className={`${actionLinkClass} h-11`}>
              <span className="flex items-center gap-2">
